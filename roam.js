...
async function loadPageData(page_id, parent_block_uid, block, arr) {
    ...
    var str = block.string;
    if (block.uid == "dpOEKZYgf") {
        debugger;
    }
    // replace all the #. classes words from the notes
    //str = str.replace(/\b#\w+\b/g, "").replace(/\s+/g, " ").trim();
    ...
    if (str.indexOf("{{[[video-timestamp") != -1) {
        ...
        //let pattern = /\{\{.*?\}\}/g;
        //let str = str.replace(pattern, "").replace(/\s+/g, " ").trim();
    }
